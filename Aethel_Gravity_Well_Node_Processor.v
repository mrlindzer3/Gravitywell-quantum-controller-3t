...
module Aethel_Gravity_Well_Node_Processor (
    ...
    localparam [15:0] KP_GAIN = 16'h0F80;
    localparam [15:0] KD_GAIN = 16'h0240;
    ...
    assign current_error = -($signed(apd_input_current));
    assign derivative    = current_error - prev_error;
    
    // Proportional-Derivative (PD) Correction Matrix Step
    assign raw_stabilization = (current_error * $signed(KP_GAIN)) + (derivative * $signed(KD_GAIN));
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            intermediate_product <= raw_stabilization * $signed(static_metric_scale);
